...
// load the before_process function from the payment modules
  ...
  $sql_data_array = array('customers_id' => $customer_id,
						  ...
						  'customers_reference' => $order->info['reference'],

                          'customers_company' => $order->customer['company'],

                          'customers_vat_number' => $order->customer['vat_number'],

                          'customers_ean_number' => $order->customer['ean_number'],
                          ...
                          'delivery_ean_number' => $order->delivery['ean_number'],
                          ...
                          'billing_ean_number' => $order->billing['ean_number'],
                          ...
                          /*'giftMethod' => $giftwrap_card,

                          'giftCard' => $giftwrap_card,

                          'giftMessage' => $giftwrap_message,*/

                          'giftMethod' => $order->info['giftwrap_method'],

                          'giftCard' => $order->info['giftwrap_card'],

                          'giftMessage' => $order->info['giftwrap_message'],
...
// QuickPay changed end
  ...
  if (preg_match('/gls_(?P<gls_shopid>\d+)/', $shipping['id'], $matches)){
    ...
    $delivery_name = 'GLS Pakkeshop nr. '.$matches['gls_shopid']; // just in case there is no answer from gls website

    $delivery_street_address = $delivery_postcode = $delivery_city = '';
    ...
    if ($pakkeshop_available) {
        ...
        for ($i=0; $i<count($pakkeshop_available); $i++) {
            ...
            if ($matches['gls_shopid'] == $pakkeshop['Number']) {
                ...
                $delivery_name              = $pakkeshop['CompanyName'] . ', ' . $pakkeshop['Streetname2'];

                $delivery_street_address    = $pakkeshop['Streetname'];

                $delivery_postcode          = $pakkeshop['ZipCode'];

                $delivery_city              = $pakkeshop['CityName'];
    ...
    $gls_data_array = array('delivery_name' => $delivery_name, 
    ...
    tep_db_perform(TABLE_ORDERS, $gls_data_array, 'update', 'orders_id = ' . $insert_id);
...
//++++ QT Pro: End Changed Code
        ...
        if ( ($stock_left < 1) && (STOCK_AUTO_SET_OFFLINE == 'Ja') ) {
...
// PWA - un comment this line to use this option
...
// unregister session variables used during checkout
  ...
  tep_session_unregister('reference');

  // BOF GiftWrap

  tep_session_unregister('giftwrap_method');

  tep_session_unregister('giftwrap_message');

  tep_session_unregister('giftwrap_card');
