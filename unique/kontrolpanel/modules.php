...
<?php
  ...
  switch ($action) {
	...
	$addcount = "";
if($value['title'] == "MODULE_ADMIN_LABEL_GROUP"){
	$addcount = $n.". ";
	$n++;
}else{
	$addcount = "";
}
if(strstr($value['title'],"MODULE_ADMIN")){
		$value['title'] = constant($value['title']);
		$value['description'] = constant($value['description']);
	}
   //quickpay added end			  
  //quickpay changed start 
        $keys .= '<b>'.$addcount . $value['title'].'</b><br>' . $value['description'] . '<br>';
